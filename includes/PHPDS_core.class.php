...
class PHPDS_core extends PHPDS_dependant
{
    ...
    public function setDefaultNodeParams()
    {
        ...
        if (!empty($navigation[$configuration['m']])) {
            ...
            try {
                switch ($menu_case) {
                    ...
                    case 4:
                        ...
                        $result = $this->loadFile($navigation[$menu_id]['menu_link']);
                        if (false == $result) {
    ...
    public function webPath($object)
    {
        if (is_array($object)) {
            ...
            return '/plugins/'.$this->activePlugin().'/'.$object;
        }
        return false;
    }
