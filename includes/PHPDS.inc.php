...
define('phpdevshell_version', 'PHPDevShell V 3.2.0-Stable-DB-3140');
 ...
 *
 * NOTE: there is an "embed" mode which is used for unit testing - you should not try to use it
 *
 * For version 4.0, these deprecated methods have been removed:
 * - _log()
 * - info()
 *
 * @date 20090427
...
class PHPDS
{
    ...
    protected $classes;
    /**
     * PHPDS is used standalone (false) or for unit testing (true)
     *
     * Don't use
     *
     * @var boolean
     ...
     * @date 20131203 (1.1.3) (greg) the current directory is set in a variable $HERE
     *
     * @param string $path absolute file name
     * @param PHPDS_array $configuration the configuration array to inject into
     *
     * @return bool whether it's successful
     */
    protected function includeConfigFile($path, &$configuration)
    {
        ...
        $HERE = realpath(dirname($path));
        $HERE = PU_SafeSubpath($HERE).DIRECTORY_SEPARATOR;
     ...
     * @date 20090521	Added
     * @date 20091006 Changed to dual list of files (used/missing)
     * @date 20121212 (v1.1) (greg) now using PU_SafeSubpath()
     *
     * @param string $filename	short file name
     * @param PHPDS_array $configuration the configuration array to inject into
     *
     * @return bool whether the file was correctly included or not
     */
    protected function loadConfigFile($filename, &$configuration)
    {
        ...
        $filename = PU_SafeSubpath($filename.'.config.php', '', 'config');
        if (!$filename) {
            $filename = PU_SafeSubpath($filename.'.local.config.php', '', 'config');
        }

        return $filename ? $this->includeConfigFile($filename, $configuration) : false;
    }
     ...
     * @date 20130304 (1.2) (greg) moved autoload to beginning
     * @date 20130306 (1.2.1) (greg) added exception in case autoload fails
     *
    ...
    protected function loadConfig()
    {
        ...
        if (!$success) {
            throw new PHPDS_exception('Unable to register AutoLoader');
        }

        // starts with an empty array which will contain the actual names of the configurations files used
        $configuration = $this->_factory('PHPDS_configuration');
        ...
        // store the $configuration inside the main Skul
    ...
    protected function configSession()
    {
        ...
        try {
            if ($this->embedded) {
                ...
                $this->configuration['session_name'] = empty($session_name) ? 'TESTING' : $session_name;
            } else {
                if (!empty($this->configuration['absolute_url']))
     ...
     *
     * Note: previous values maybe be over-written but not deleted
     *
     * @date 			20100219
     * @author		greg
     * @version		1.0
    ...
    protected function configCoreSettings()
    {
        ...
        if (!isset($this->configuration['use_reflection'])) {
            $this->configuration['use_reflection'] = class_exists('ReflectionMethod');
        }
...
//        $this->PHPDS_core()->loadDefaultPluginLanguage(); /////////////////////
     ...
     *
     * @date 20130408 (1.0.1) (greg) support for PHPDS_configuration as class type
     * @date 20090427
     * @version	1.0.1
     *
    ...
    public function PHPDS_configuration($configuration = null)
    {
        if (empty($this->configuration)) {
            if (is_a($configuration, 'PHPDS_configuration')) {
                $this->configuration = new PHPDS_array($configuration);
                $this->configuration['time'] = time();
            } else {
                throw new PHPDS_exception('Configuration is NOT lazy loaded!');
            }
        }
     ...
     *
     * The goal of this function is to be called all thorough the code to be able to track bugs.
     *
     * @param string $data
     */
     ...
     *
     * Will probably be deprecated in 4.0
     *
     * @date 20090427
     * @version	1.0
     *
    ...
    /**
     * Allow access to the URL routing subsystem
     *
     * @version 1.0
     *
    ...
    public function PHPDS_router()
    {
        if (empty($this->router)) {
            $this->router = $this->_factory(('&PHPDS_router'));
        }
        return $this->router;
    }
     ...
     * @date 20130226 (1.3) (greg) added "includes/database" to the searched folders
     * @date 20130408 (v1.3.1) (greg) fixed a path typo with "local" folders
     * @version 1.3.1
     * @author jason
     * @author greg <greg@phpdevshell.org>
     * @param $class_name
     * @return boolean irrelevant, it's a callback for the new() operator (however we return true on success, false otherwise)
     */
 ...
 * @property PHPDS_classFactory $classes
...
{
    /**
     * The object this object depend on. Ultimately up the chain it should be the main PHPDS instance
     * @var $dependance PHPDS_dependant|PHPDS
     */
    protected $dependance;
    /**
     * The private debug instance of this object (with domain selection)
     * @var $debugInstance PHPDS_debug the debug object
     */
    private $debugInstance;
    /**
     * Holds a dependent extended object to appear as parent.
     * @var $parent object|string
     */
    protected $parent;
    /**
     ...
     *
     * @date 20140608 (1.2) (greg) caches the root to improve performances
    ...
    public function PHPDS_dependance($dependance = null)
    {
        ...
        if (!is_object($this->dependance)) {
            throw new PHPDS_exception("Dependency error: dependance is not an object.");
        }
        if (empty($this->root)) {
            $this->root = method_exists($this->dependance, 'PHPDS_dependance') ? $this->dependance->PHPDS_dependance() : $this->dependance;
        }
        return $this->root;
    }
    ...
    public function __get($name)
    {
        ...
        try {
            ...
            throw new PHPDS_Exception("Can't get any '$name' (maybe dependency is wrong).", 0, $e);
        }
    }
    ...
    public function __isset($name)
    {
        ...
        try {
           ...
           // throw new PHPDS_Exception("Non-existent field '$name' (maybe dependency is wrong).'");
        } catch (Exception $e) {
            //throw new PHPDS_Exception("Can't get any '$name' (maybe dependency is wrong).", 0, $e);
        }
     ...
     *
     * @date 20100412 (v1.0.1) (greg) deal with new style naming (starting with PHPDS_)
    ...
    {
        if (is_a($this, 'debug')) {
            $this->debug($data); // todo: is it valid anymore?
        } else {
            $this->debugInstance()->debug($data);
        }
    }
     ...
     * @param mixed ... optional class specific parameters
     *
...
class PHPDS_classFactory extends PHPDS_dependant
{
     ...
     * @var array
     */
    protected $PluginClasses = array('#' => array());
    /**
     ...
     *
     * Note that classes added "on the fly" supersede the registry stored in the database
     *
     * If the class name starts with a #, a singleton instance will be initialized after config is done
     *
     * @version 2.0
     * @since 3.1.2
     * @author greg <greg@phpdevshell.org>
     ...
     * @param string $classAlias an alternate name for this class
    ...
    {
        // if the class is registered as auto-initialized class, it's added to the list
        if ($className[0] == '#') {
            $className = substr($className, 1);
            $this->PluginClasses['#'][] = $className;
        }
    ...
    public function loadRegistry()
    {
        ...
        if (is_array($this->PluginClasses['#'])) {
            foreach($this->PluginClasses['#'] as $classname) {
                $this->factorClass('&'.$classname, null, $this->PHPDS_dependance());
            }
        }
    }
     ...
     *
     * @param string|array          $classname name of the class to instantiate, or parameter array
     ...
     * @throws PHPDS_exception
     *
     * @date    20130305 (1.1.4) (greg) removed commented out code ; initialized $o to conform to quality control
    ...
    public function factorClass($classname, $params = null, $dependancy = null)
    {
        ...
        try {
            ...
            if (class_exists($classname, true)) {
                ...
                throw new PHPDS_exception("Unable to factor a new \"$classname\" with \"$c\".");
            } else {
    ...
    /**
     * Returns the class name to use, i.e. a substitute name if exists, or else the original name
     *
     * @version 1.0
     *
    ...
    {
        $c = $this->classParams($class_name);
        return $c ? $c['class_name'] : $class_name;
    }
...
class PHPDS_configuration extends PHPDS_array
{
    /*public function addHost($host, $file)
    {
        if (!empty($host)) {
