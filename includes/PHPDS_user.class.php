...
/**
 * This class represent a user in PHPDevShell
 *
 * A default instance for the current user is available in $this->user for all dependant objects
 * but you can create more user objects to fit your needs.
 *
 * As a general rule, try to always use method from this class and never access the database directly.
 *
 */
class PHPDS_user extends PHPDS_dependant
{
     ...
     *
     * @date 20130220 (greg) (v1.1) cache the result in the field "mergeRoles"
     *
     * @version 1.1
	 *
	 ...
	 */
	public function getRoles($user_id = false, $return_as_array = false)
	{
        if (empty($this->mergeRoles)) {
		    $this->mergeRoles = $this->db->invokeQuery('USER_getRolesQuery', $user_id, $return_as_array);
        }
	 ...
	 *
     * @date 20150205 (1.1) (greg) moved the "need process" test to StandardLogin; also use a singleton
	 * @date 2011-06-20
     *
     * @version 1.1
	 * @author Jason Schoeman, Greg <greg@phpdevshell.org>
	 */
