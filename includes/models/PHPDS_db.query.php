    ...
    public function run(&$parameters = null)
    {
        ...
        if (! empty($log_array) && $this->configuration['system_logging'] == true) {
            ...
            foreach ($log_array as $logged_data) {
                ...
                if (empty($logged_data['file_name'])
                    && !empty($navigation[$this->configuration['m']]['menu_link']))
                {
                    $logged_data['file_name'] = $navigation[$this->configuration['m']]['menu_link'];
                } else {
                    $logged_data['file_name'] = ___('N/A');
                }
                if (empty($logged_data['menu_name'])
                    && !empty($navigation[$this->configuration['m']]['menu_name']))
                {
                ...
                if (!empty($logged_data['log_type']) || !empty($logged_data['log_description'])) {
                    $database_log_string .= PU_sprintfn(
                        '(NULL, %(log_type)s, %(log_description)s, %(timestamp)s, %(user_id)s, %(logged_by)s, %(menu_id)s, %(file_name)s, %(menu_name)s, %(user_ip)s),',
                        $this->protectArray($logged_data, "'"));
                }
            }
            $database_log_string = rtrim($database_log_string, ',');
        }
        return empty($database_log_string) ? false : $this->query($database_log_string);
    }
}
...
class DB_doesRecordExistQuery extends PHPDS_query
{
    ...
    public function invoke($parameters = null)
    {
        ...
        if (is_array($search_column_names) && is_array($search_field_values)) {
            ...
            // Trim
            $MATCH_in_db = PU_rightTrim($MATCH_in_db, ' OR ');
