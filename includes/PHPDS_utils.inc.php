...
/**
 *
 * For version 4.0, these deprecated functions have been removed:
 * - PU_array_compact() - use PU_arrayCompact()
 */
...
function PU_inArray($needle, $haysack)
{
//    return in_array($needle, $haysack);
    foreach ($haysack as $k => $v) {
        if ($v === $needle) {
            return true;
        }
 ...
 * @date 20120630 (greg) (v2.0) made it recursive; improved html
 * @date 20130409 (greg) (2.1) added a '_dumped_' property to avoid infinite recursivity ; $added styles
 *
 * @param array $a
 * @param string $title
 * @param boolean $htmlize (default to false) if true html is escaped to be displayed as source
 * @param array $styles some styles to apply to some elements
 *
 * @return string
 */
function PU_dumpArray($a, $title = null, $htmlize = false, $styles = array(), $root = null)
{
    if (!isset($root)) {
        $root = $a;
    }
	...
	if (is_object($a)) {
        if (!empty($a->_dumped_)) {
            return '<span class="array_ref" data-id="'. spl_object_hash($a).'">...</span>';
        }
        $a->_dumped_ = true;
//        $a = array($a);
    } elseif (!is_array($a)) {
	...
	if (count($a) == 0) {
        ...
        $class = 'array_dump '.(!empty($styles['heads']) ? $styles['heads'] : 'array_head');
		$s .= '<ul class="'.$class.'">';
		foreach ($a as $k => $e) {
            if ($k != '_dumped_') {
                $valueType = gettype($e);
                $realType = $valueType;
                switch ($valueType) {
                    case 'array': $valueType .= ', '.count($e).' elements'; break;
                    case 'string':
                            $valueType .= ', '.strlen($e).' chars, '.mb_detect_encoding($e);

                        break;
                    case 'object': $valueType = '<span class="array_object">'.$valueType.' of class "'.get_class($e).'"</span>'; break;
                    case 'boolean': $e = $e ? 'true' : 'false'; break;
                }
                $id = is_object($e) && (($a === $root) || !(is_array($root) && PU_inArray($e, $root))) ? 'id="dump_' .spl_object_hash($e).'"' : '';
                $s .= '<li '.$id.'>'
                    .'<span class="array_key"><span class="array_grey">[&nbsp;</span>'.$k.'<span class="array_grey">&nbsp;]&nbsp;=&gt;</span></span>'
                    .'&nbsp;<span class="array_type">('.$valueType.')</span>&nbsp;';

                // depending on the object being an array, an object, or something else, we recurse or just display it
                if (is_array($e)) {
                    $e = PU_dumpArray($e, null, $htmlize, array('heads' => 'array_child'), $root);
                } elseif (is_object($e)) {
                    if (is_array($root) && ($root != $a) && PU_inArray($e, $root)) {
                        if (is_object($e)) {
                            $e = '<span class="array_ref" data-id="' . spl_object_hash($e) . '">(top object)</span>';
                        } else {
                            $e = '(top array)';
                        }
                    } else {
                    ...
                    if (($realType == 'string') && PU_SafeSubpath((string)$e)) {
                        $realType = 'path';
                    }
                    if ($htmlize) {
                        $e = htmlentities($e);
                ...
                $s .= '<span class="array_value value_'.$realType.'">' . (string)$e . '</li>';
            }
		}
		$s .= '</ul>';
	}
 ...
 * @return array
 */
function PU_arrayCompact(array $a)
{
	foreach($a as $k => $v) {
 ...
 * ));
 *
 * If an argument is array, it's flattened with commas: array('a', 'b') => "a, b"
 *
 * @author nate at frickenate dot com
 *
 * @version 1.1
 * @date 20120724 (v1.0.1) (greg) cleaned up exception
 * @date 20130526 (1.1) (greg) added support for array arguments
 *
 ...
 *
 * @return string|boolean result of sprintf call, or bool false on error
 */
function PU_sprintfn($format, array $args = array())
{
	try {
		...
		for ($pos = 0; preg_match('/(?<=%)\(([a-zA-Z_]\w*)\)/', $format, $match, PREG_OFFSET_CAPTURE, $pos);) {
            ...
            // if an argument to be used is an array, let's flatten it
            if (is_array($args[$arg_key])) {
                $args[$arg_key] = implode($args[$arg_key], ', ');
            }
 ...
 * @date 20130223 (1.2) (greg) added default connector class name
 *
 ...
 * @param string $dbConfigName Specifies the name of the database configuration to use, leave blank if not sure.
 * @return array The database settings
 */
function PU_GetDBSettings($configuration, $dbConfigName = '')
{
    try {
        if (!empty($configuration['database_name'])) {
            ...
            return array(
                ...
                'connector'  => 'PHPDS_legacyConnector'
            );
        } else {
            ...
            if (!empty($databases)) {
                if (empty($dbConfigName)) {
                    $dbConfigName = (string)$configuration['master_database'];
                }

                if (isset($databases[$dbConfigName])) {
                    return $databases[$dbConfigName];
                }
            }
        }
    } catch (Exception $e) {
	    throw new PHPDS_databaseException(_('Error trying to provide the required database settings'));
    }
 ...
 * @version 1.1
 *
 * @date 20130224 (1.1) (greg) added check for non-defined $_SESSION
 * 
 * @return string 
 */
function PU_PackEnv()
 ...
 * @date 20121213 (v1.2) (greg) added parameter subfolder
 *
 * @param string $path the path to check (absolute or relative)
 * @param string $root (optional) the root to check against (default if empty)
 * @param string $subfolder (optional) a folder to add to the root if the path is relative
 *
 * @return string|false the actual path or false 
 */
function PU_SafeSubpath($path, $root = '', $subfolder = '')
{
    if (empty($path)) {
        return false;
    }
	...
	if (substr($path, 0, 1) != '/') {
		$path = $root.DIRECTORY_SEPARATOR.$subfolder.DIRECTORY_SEPARATOR.$path;
	}
	$path = realpath($path);
