...
class MenuItemAdmin extends PHPDS_controller
{
    ...
    public function execute()
    {
        ...
        $edit_menu_id = empty($this->_GET['em']) ? null : $this->_GET['em'];
        ...
        if (!empty($this->security->post['save']) || !empty($edit_menu_id) || !empty($this->security->post['new'])) {
            ...
            if (!empty($this->security->post['save']) || !empty($this->security->post['new'])) {
                ...
                // menu_type
                if (empty($edit['menu_type'])) {
                    $edit['menu_type'] = 1;
                }
                // new_window
                if (empty($edit['new_window'])) {
                    $edit['new_window'] = 0;
                }

                // permission
                if (!empty($edit['permission'])) {
                    foreach ($edit['permission'] as $role_id) {
                        $selected_user_roles[$role_id] = 'selected';
                    }
                }
            }
            // menu_id
            if (!empty($edit['menu_id'])) {
                ...
                $edit['menu_id'] = PU_safeName($edit['menu_id']);
            ...
            switch ($edit['menu_type']) {
                ...
                case 12:
                    ...
                    if (is_file($query_file)) {
                        $query_found = $template->icon('database--plus', _('Model file available'))
                            .' '.$query_file;
                    } else {
                    ...
                    if (empty($edit['layout'])) {
                        ...
                        if (is_file($view_file)) {
                            $view_found = $template->icon('eye--plus', _('View file available'))
                            .' '.$view_file;
                        } else {
            ...
            if ((!empty($this->security->post['save']) || !empty($this->security->post['new'])) && $this->user->isRoot()) {
                ...
                // Define.
                $edit['extend'] = false;
                $edit['alias'] = PU_safeName($edit['alias']);
                ...
                if (empty($error)) {
                    ...
                    if (!empty($edit['inherit_permissions'])) {
                        $user_role_id_db .= "(-1, '{$edit['menu_id']}')";
                        $inherit_rights = true;
                    }
        ...
        if (!empty($selected_user_roles[-1])) {
            $inherit_rights = true;
            unset($selected_user_roles[-1]);
        }
        ...
        $view->set('inherit_rights', $inherit_rights);
